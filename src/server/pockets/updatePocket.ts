...
export const updatePocket = async(pocket:PocketModel,balanceId: string):Promise<PocketModel | null> => {
    const headers = new Headers();
    headers.append("Content-Type", "application/json");

    try {
        const response = await fetch(`${import.meta.env.VITE_SPRING_BACKEND}/pocket/update`,{
            method: "PUT",
            headers,
